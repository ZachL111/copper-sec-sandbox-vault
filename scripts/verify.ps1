...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-sec-sandbox-detail.ps1

php tests/test_domain_review.php

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-sec-sandbox-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-sec-sandbox-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper_sec_sandbox_vault-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
